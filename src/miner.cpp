...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("CPRAMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("CPRAMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("CPRAMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("capracoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("CPRAMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running CPRAMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadCPRAMiner() exception");
    } catch (...) {
        LogPrintf("ThreadCPRAMiner() exception");
    }

    LogPrintf("ThreadCPRAMiner exiting\n");
}
